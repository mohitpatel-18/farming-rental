...
export default function MyToolsPage() {
  ...
  // normalize tools array (safe fallback)
  ...
  const startEdit = (tool) => {
    ...
    // support either field name 'hourly' or 'pricePerHour'
    const price = tool.hourly ?? tool.pricePerHour ?? "";
    setEditForm({
      title: tool.title || "",
  ...
  const saveEdit = () => {
    try {
      ...
      const updated = all.map((t) => {
        if (t.id === editingId) {
          // keep both keys in sync: update hourly and pricePerHour
          const numericPrice = Number(editForm.pricePerHour || 0);
          return {
            ...t,
      ...
      // if your ShopContext reads localStorage on load, refresh to pick changes
  ...
  const confirmAndRemove = (id) => {
    ...
    // call context remove if available
    try {
      removeTool?.(id);
    } catch (e) {
      // ignore if not provided
      console.warn("removeTool threw", e);
    }
    // quick refresh to rehydrate
  ...
  return (
    <div style={{ padding: 20 }}>
      ...
      {editingId && (
        <div style={editorBox}>
          ...
          <input
            ...
            placeholder="paste image DataURL"
          />

          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <button style={primaryBtn} onClick={saveEdit}>
              Save
            </button>
            <button style={outlineBtn} onClick={cancelEdit}>
              Cancel
            </button>
          </div>
        </div>
      ...
      <div style={{ marginTop: 12 }}>
        {myTools.length === 0 ? (
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {myTools.map((t) => {
              const price = t.hourly ?? t.pricePerHour ?? 0;
              return (
                <div key={t.id} style={card}>
                  ...
                  <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", gap: 12 }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ marginTop: 6, fontWeight: 700, color: "#1f7a3a" }}>₹{price}/hour</div>
                    </div>

                    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginLeft: 12 }}>
                      <button style={smallBtn} onClick={() => startEdit(t)}>
                        Edit
                      </button>
                      <button style={removeBtn} onClick={() => confirmAndRemove(t.id)}>
                        Remove
                      </button>
                    </div>
                  </div>
      ...
      {/* Floating add button (component handles modal + adding) */}
      <AddToolButton />
    </div>
  );
}
